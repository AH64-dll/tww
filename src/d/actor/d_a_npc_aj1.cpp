...
void daNpc_Aj1_c::setMtx(bool i_setMtx) {
    mpMorf->getModel()->setBaseTRMtx(mDoMtx_stack_c::trans(current.pos.x, current.pos.y, current.pos.z));
...
void daNpc_Aj1_c::isEventEntry() {
    dComIfGp_evmng_getMyStaffId(mEventCut.getActorName(), this, 0);
}
...
s32 daNpc_Aj1_c::chk_areaIN(f32 i_radius, s16 i_angle, cXyz i_pos) {
    ...
    if (dist < i_radius && fabsf(yDiff) < 500.0f && abs(angleDiff) < i_angle) {
        return 1;
    }
    return 0;
}
...
u8 daNpc_Aj1_c::demo() {
    if (demoActorID == 0) {
        ...
        dDemo_actor_c* demoActor = dComIfGp_getDemoManager()->getActor(demoActorID);
...
BOOL daNpc_Aj1_c::_draw() {
    ...
    dSnap_RegistFig(DSNAP_TYPE_NPC_AJ1, this, 1.0f, 1.0f, 1.0f);
    return TRUE;
}
...
BOOL daNpc_Aj1_c::_execute() {
    ...
    if (demo() == 0) {
        s32 staffIdx = -1;
        if (dComIfGp_event_runCheck() && dComIfGp_getEvent().checkCommandTalk() == 0) {
        ...
        play_animation();
        mObjAcch.CrrPos(dComIfGp_getBg());
    }
    eventOrder();
    m70E.x = current.angle.x;
    m70E.y = current.angle.y;
    ...
    field_0x1B5 = dComIfGp_Bgsp().GetRoomId(mObjAcch.m_gnd);
    field_0x1B6 = dComIfGp_Bgsp().GetPolyColor(mObjAcch.m_gnd);
    setMtx(FALSE);
    flw_pa_pun();
    del_pa_aka();
    flw_pa_aka();
