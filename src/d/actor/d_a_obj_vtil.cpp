...
const int daObjVtil_c::l_daObjVtil_bdl_idx_table[5] = {
    dRes_INDEX_VTIL_BDL_VTIL1_e,
    dRes_INDEX_VTIL_BDL_VTIL2_e,
    dRes_INDEX_VTIL_BDL_VTIL3_e,
...
cPhs_State daObjVtil_c::_create() {
    ...
    m60C = daObj::PrmAbstract(this, PRM_TYPE_W, PRM_TYPE_S);
    if (m60C == 0xF || m60C == -1) {
        m60C = 0;
    }
...
void daObjVtil_c::calc_throw(float* gravity, float* param_2, float* param_3) const {
    ...
    if (const_cast<dBgS_ObjAcch&>(mAcch).ChkSeaIn()) {
        ...
        f32 f2 = 1.0f - fVar1;
        *param_2 = fVar1 * 0.2f + f2 * 0.002f;
        *param_3 = fVar1 * 0.02f + f2 * 0.0002f;
...
void daObjVtil_c::hit_co() {
    ...
    if (hit) {
        ...
        if (hit) {
            tell_agb_attack();
            daObj::HitSeStart(&eyePos, current.roomNo, &mCyl, 0xD);
            set_sound(0x96, 5);
...
BOOL daObjVtil_c::check_sink() {
    ...
    if (mAcch.ChkSeaIn() && mAcch.GetSeaHeight() > fopAcM_GetPosition_p(this)->y + 70.0f) {
        ret = TRUE;
    }
...
BOOL daObjVtil_c::check_sink_end() {
    ...
    if (mAcch.ChkSeaIn() && mAcch.GetSeaHeight() > fopAcM_GetPosition_p(this)->y + 160.0f + 50.0f) {
        ret = TRUE;
    }
...
void daObjVtil_c::make_vib() {
    BOOL temp = check_circle() & 1;
    dComIfGp_getVibration().StartShock(temp + 1, 1, cXyz(0.0f, 1.0f, 0.0f));
}
...
BOOL daObjVtil_c::check_circle() {
    ...
    cXyz playerPos = player->current.pos - current.pos;
    cXyz circleCenter(playerPos.x, 0.0f, playerPos.z);
    f32 dist = std::sqrtf(PSVECSquareMag(&circleCenter));
    BOOL ret = FALSE;
    if (dist <= 190.0f) {
        ret = TRUE;
    }
...
bool daObjVtil_c::_draw() {
    ...
    if (!fopAcM_checkCarryNow(this)) {
        dComIfGd_setSimpleShadow2(&current.pos, mAcch.GetGroundH(), 57.0f, mAcch.m_gnd, 0, 1.0f,
