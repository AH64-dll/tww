...
void daObjFirewall_c::particle_delete() {
    for (int i = 0; i < 6; i++) {
        JPABaseEmitter* prt = mParticles[i];
        if (prt != NULL) {
            prt->mMaxFrame = -1;
            prt->mFlags |= 1;
            mParticles[i] = NULL;
    ...
    for (int i = 0; i < 5; i++) {
        JPABaseEmitter* prt = mParticles2[i];
        if (prt != NULL) {
            prt->mMaxFrame = -1;
            prt->mFlags |= 1;
            mParticles2[i] = NULL;
    ...
    if (phase == cPhs_COMPLEATE_e) {
        if (fopAcM_entrySolidHeap(this, solidHeapCB, 0x1420)) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                f32 sx = 1000.0f * scale.x;
                mSePos[i].x = current.pos.x + sx * sin;
                mSePos[i].y = current.pos.y;
                mSePos[i].z = current.pos.z + sx * cos;
            }
            m10E0 = 1;
            mParam = daObj::PrmAbstract(this, PRM_SWSAVE_W, PRM_SWSAVE_S);
            if (dComIfGs_isEventBit(0x3520) == 1) {
                m10E4 = 1;
